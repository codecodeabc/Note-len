...
同步组件的实现聚合了同步器（AQS），并通过重写重写同步器（AQS）中的方法实现同步组件的同步语义（关于同步组件的实现层级结构可以[看这篇文章](https://juejin.cn/post/6844903601534418958)，AQS的底层实现分析可以[看这篇文章](https://juejin.cn/post/6844903601538596877)）。因此，写锁的实现依然也是采用这种方式。在同一时刻写锁是不能被多个线程所获取，很显然写锁是独占式锁，而实现写锁的同步语义是通过重写AQS中的tryAcquire方法实现的。源码为:
...
![读写锁的读写状态设计.png](https://raw.githubusercontent.com/codecodeabc/Note-len/main/img/202111081715120.webp)
...
作者：你听___
链接：https://juejin.cn/post/6844903601546985485
来源：稀土掘金

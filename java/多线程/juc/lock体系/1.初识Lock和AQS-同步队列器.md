...
![concurrent目录结构.png](https://raw.githubusercontent.com/codecodeabc/Note-len/main/img/202111081124150.webp)concurrent目录结构.png

其中包含了两个子包：atomic以及lock，另外在concurrent下的阻塞队列以及executors,这些就是concurrent包中的精华，之后会一一进行学习。而这些类的实现主要是依赖于volatile以及CAS（关于volatile可以看[这篇文章](https://juejin.cn/post/6844903601064640525)，关于CAS可以看[这篇文章的3.1节](https://juejin.cn/post/6844903600334831629)），从整体上来看concurrent包的整体实现图如下图所示：

![concurrent包实现整体示意图.png](https://raw.githubusercontent.com/codecodeabc/Note-len/main/img/202111081124819.webp)concurrent包实现整体示意图.png
...
![AQS可重写的方法.png](https://raw.githubusercontent.com/codecodeabc/Note-len/main/img/202111081124763.webp)AQS可重写的方法.png

在实现同步组件时AQS提供的模板方法如下图：

![AQS提供的模板方法.png](https://raw.githubusercontent.com/codecodeabc/Note-len/main/img/202111081124832.webp)AQS提供的模板方法.png
...
![mutex的执行情况.png](https://raw.githubusercontent.com/codecodeabc/Note-len/main/img/202111081129500.webp)mutex的执行情况.png
...
总的来说，同步组件通过重写AQS的方法实现自己想要表达的同步语义，而AQS只需要同步组件表达的true和false即可，AQS会针对true和false不同的情况做不同的处理，至于底层实现，可以[看这篇文章](https://juejin.cn/post/6844903601538596877)。



作者：你听___
链接：https://juejin.cn/post/6844903601534418958
来源：稀土掘金

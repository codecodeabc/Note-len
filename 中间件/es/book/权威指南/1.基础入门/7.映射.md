...
拷贝为 curl[在 Sense 中查看](http://localhost:5601/app/sense/?load_from=https://www.elastic.co/guide/cn/elasticsearch/guide/current/snippets/052_Mapping_Analysis/45_Mapping.json) 
...
`tweet` 域产生**两个词条 `black` 和 `cat`** ， **`tag` 域产生单独的词条 `Black-cats`** 。换句话说，我们的映射正常工作。
